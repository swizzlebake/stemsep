...
void BandFilter::prepareCoefficients(float freqHz, float Q, float gainDB)
{
    ...
    ta2 = (1.0 - alpha)  * a0_inv;

    tGainLin_ = std::pow(10.0, (double)gainDB / 20.0);
}
                              ...
                              float* outL, float* outR, int numSamples)
{
    ...
    const double dGain = (tGainLin_ - gainLin_) * inv;
    ...
    for (int n = 0; n < numSamples; ++n)
    {
        ...
        for (int ch = 0; ch < 2; ++ch)
        {
            ...
            auto& s = state[ch];
            // b1 is always 0 for BPF, so the x[n-1] term is omitted
            const double y = b0 * x  + b2 * s.x2
                              - a1 * s.y1 - a2 * s.y2;
            s.x2 = s.x1; s.x1 = x;
            s.y2 = s.y1; s.y1 = y;
            if (ch == 0) outL[n] += (float)(y * gainLin_);
            else         outR[n] += (float)(y * gainLin_);
        }
    }
...
float BandFilter::getMagnitudeForFrequency(float freqHz, double sampleRate) const
{
    ...
    // b1 = 0 for BPF
    const double numR = tb0 + tb2 * c2;
    const double numI =     - tb2 * s2;
    ...
    return (float)((numMag / (denMag + 1e-10)) * tGainLin_);
}
